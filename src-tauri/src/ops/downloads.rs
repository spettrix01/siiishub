...
/// the app also sets it as the folder's `metadata::custom-icon`, which
/// Nautilus, Nemo and Caja display like Explorer does with `desktop.ini`.
#[cfg(not(windows))]
mod unix_cover {
    use std::path::Path;
...
pub async fn start(state: &Arc<AppState>, args: DownloadStartArgs) -> CmdResult<DownloadRecord> {
...
pub async fn add_local(state: &AppState, path: String) -> CmdResult<DownloadRecord> {
...
pub fn list(state: &AppState) -> Vec<DownloadEntry> {
    let mut out = Vec::new();
...
pub async fn remove(state: &AppState, id: String) -> CmdResult<()> {
    let record = state
        .downloads
        .list()
        .into_iter()
...
pub async fn pause(state: &AppState, id: String) -> CmdResult<()> {
...
pub async fn resume(state: &Arc<AppState>, id: String) -> CmdResult<()> {
    let record = find_download_record(state, &id)?;
    match record.kind {
...
/// What the player opens for a downloaded title: a `file://` URL for a file
/// on disk, the torrent session's stream URL for a torrent still in it.
pub async fn play(state: &AppState, id: String, file_index: Option<usize>) -> CmdResult<String> {
    let record = find_download_record(state, &id)?;
...
pub fn parse_torrent_file(bytes: &[u8]) -> CmdResult<ParsedTorrent> {
    parse_torrent_bytes(bytes).map_err(err)
}

/// The folder holding a download (created if missing), for the app to show
/// it in the file manager.
pub async fn folder(state: &AppState, id: String) -> CmdResult<PathBuf> {
    let record = find_download_record(state, &id)?;
...
pub async fn files(state: &AppState, id: String) -> CmdResult<Vec<TorrentFileInfo>> {
    let record = find_download_record(state, &id)?;
